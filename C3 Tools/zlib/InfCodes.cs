...
namespace C3Tools.zlib
{
	...
	sealed class InfCodes
	{
		...
		internal int proc(InfBlocks s, ZStream z, int r)
		{
			...
			var p = z.next_in_index; var n = z.avail_in; var b = s.bitb; var k = s.bitk;
			var q = s.write; var m = q < s.read?s.read - q - 1:s.end - q;
		...
		internal int inflate_fast(int bl, int bd, int[] tl, int tl_index, int[] td, int td_index, InfBlocks s, ZStream z)
		{
			...
			// initialize masks
			var ml = inflate_mask[bl];
			var md = inflate_mask[bd];
			...
			do 
			{
				...
				do 
				{
					...
					if ((e & 64) == 0)
					{
						t += tp[(tp_index + t) * 3 + 2];
						t += (b & inflate_mask[e]);
					    if ((e = tp[(tp_index + t)*3]) != 0) continue;
					    b >>= (tp[(tp_index + t) * 3 + 1]); k -= (tp[(tp_index + t) * 3 + 1]);
