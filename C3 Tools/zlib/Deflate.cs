...
namespace C3Tools.zlib
{
 ...
 nextlen = tree[(n + 1) * 2 + 1];
		...
		internal bool _tr_tally(int dist, int lc)
		{
		    ...
		    if ((last_lit & 0x1fff) != 0 || level <= 2) return (last_lit == lit_bufsize - 1);
		...
		internal void  compress_block(short[] ltree, short[] dtree)
		{
		    ...
		    if (last_lit != 0)
			{
				do 
				{
					var dist = ((pending_buf[d_buf + lx * 2] << 8) & 0xff00) | (pending_buf[d_buf + lx * 2 + 1] & 0xff); // distance of matched string
					var lc = (pending_buf[l_buf + lx]) & 0xff; // match length or unmatched char (if dist == 0)
 lx++;
					...
					{
						// Here, lc is the match length - MIN_MATCH
						int code = Tree._length_code[lc]; // the code to send
						
						send_code(code + LITERALS + 1, ltree); // send the length code
						var extra = Tree.extra_lbits[code]; // number of extra bits to send
						if (extra != 0)
						{
		...
		internal int deflate_stored(int flush)
		{
			...
			while (true)
			{
			    ...
			    if (strstart - block_start < w_size - MIN_LOOKAHEAD) continue;
			    flush_block_only(false);
			    if (strm.avail_out == 0)
			        return NeedMore;
			}
		...
		internal void  fill_window()
		{
		    do 
			{
				var more = (window_size - lookahead - strstart); // Amount of free space at the end of the window.
			    ...
			    if (lookahead < MIN_MATCH) continue;
		...
		internal int deflate_slow(int flush)
		{
			...
			while (true)
			{
			    ...
			    if (prev_length >= MIN_MATCH && match_length <= prev_length)
				{
					...
					do 
					{
					    if (++strstart > max_insert) continue;
					    ins_h = (((ins_h) << hash_shift) ^ (window[(strstart) + (MIN_MATCH - 1)] & 0xff)) & hash_mask;
					    //prev[strstart&w_mask]=hash_head=head[ins_h];
					    hash_head = (head[ins_h] & 0xffff);
					    prev[strstart & w_mask] = head[ins_h];
		...
		internal int longest_match(int cur_match)
		{
			...
			do 
			{
				var match = cur_match; // matched string
				...
				var len = MAX_MATCH - (strend - scan); // length of current match
