...
namespace C3Tools
{
    public partial class PackCreator : Form
    {
        ...
        private const string C3Text = "Brought to you by C3. For more customs like this, visit customscreators.com.";
        ...
        private void btnFolder_Click(object sender, EventArgs e)
        {
            ...
            var folderUser = new FolderBrowserDialog {SelectedPath = txtFolder.Text, Description = "Select the folder where your CON files are"};
            txtFolder.Text = "";
            var result = folderUser.ShowDialog();
            if (result == DialogResult.OK)
            {
                txtFolder.Text = folderUser.SelectedPath;
                Tools.CurrentFolder = Path.GetDirectoryName(txtFolder.Text);
            }
            else
            {
        ...
        private void txtFolder_TextChanged(object sender, EventArgs e)
        {
            ...
            if (txtFolder.Text != "")
            {
                ...
                try
                {
                    ...
                    var inFiles = Directory.GetFiles(txtFolder.Text).ToList();
        ...
        private bool addFiles()
        {
                    ...
                    foreach (var folder in subFolders)
                    {
                        ...
                        if (songContents.Any())
                        {
                            foreach (var content in songContents)
                            {
                                ...
                                {
                                    Tools.WriteOutData(Tools.DeObfM(File.ReadAllBytes(content)), content);
                                }
            ...
            if (Directory.Exists(upgFolder))
            {
                ...
                if (checkUpg.Count() > 1)
                {
                    ...
                    foreach (var file in from file in songsInput where (Path.GetExtension(file) == ".mid")
                                         let ending = file.Substring(file.Length - 7, 7) where ending != "(1).mid" && ending != "(2).mid" && ending != "(3).mid" &&
                                             ending != "(4).mid" && ending != "(5).mid" select file)
                    {
                        filesAdded++;
        ...
        private void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            packfiles.HeaderData.ThisType = radioCON.Checked ? PackageType.SavedGame : PackageType.MarketPlace;
            ...
            if (success)
            {
                ...
                Invoke(new MethodInvoker(() => Width = 415));
        ...
        {
            if (txtDesc.Text.ToLowerInvariant() == "c3")
            {
                txtDesc.Text = C3Text;
            }
            if (txtDesc.Text.ToLowerInvariant() == "hmx")
            {
                txtDesc.Text = HmxText;
            }

            txtDesc.ForeColor = txtDesc.Text == "Created with C3 CON Tools" ? Color.LightGray : Color.Black;
        }
        ...
        {
            var folder = new FolderBrowserDialog {Description = "***ADVANCED USE ONLY***\nSelect the 'ROOT' folder\n(i.e. contains 'songs' and 'songs_upgrades' folders)"};
            folder.ShowDialog();
            var oldfiles = folder.SelectedPath;
        ...
        {
            if (txtDesc.Text == "Created with C3 CON Tools")
            {
                txtDesc.Text = "";
            }
        }
        ...
        private void txtDesc_Leave(object sender, EventArgs e)
        {
            ...
            {
                txtDesc.Text = "Created with C3 CON Tools";
            }
        }
