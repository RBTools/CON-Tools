...
namespace C3Tools
{
    public partial class SongAnalyzer : Form
    {
        ...
        private void HandleDragDrop(object sender, DragEventArgs e)
        {
            ...
            if (VariousFunctions.ReadFileType(files[0]) == XboxFileType.STFS || Path.GetExtension(files[0]).ToLowerInvariant() == ".mid")
            {
                ShowWait(true);
                InputFile = files[0];
            ...
            else switch (Path.GetExtension(files[0]).ToLowerInvariant())
            {
                case ".mogg":
                    InputFile = files[0];
                    foreach (var file in files.Where(file => Path.GetExtension(file).ToLowerInvariant() == ".mogg"))
                    {
                        MoggFiles.Add(file);
                    }
                    ShowWait(true);
            ...
            try
            {
                ...
                switch (type)
                {
                    case FileType.MIDI:
                        ...
                        break;
                    case FileType.MOGG:
                        Tools.WriteOutData(Tools.ObfM(File.ReadAllBytes(output)), output);
                        break;
                }
            }
            catch (Exception ex)
        ...
        private void AnalyzeDrums(IList<MidiEvent> track)
        {
            ...
            DrumsX = DrumsXSnare + DrumsXKick + DrumsXBlueCymbal + DrumsXBlueTom + DrumsXGreenCymbal + DrumsXGreenTom + DrumsXYellowCymbal + DrumsXYellowTom;
        ...
        private void AnalyzeMIDI(string midi)
        {
            HaveFile = true;
            ...
            Log("");
            if (!string.IsNullOrWhiteSpace(InputFile) && VariousFunctions.ReadFileType(InputFile) == XboxFileType.STFS)
            {
                var type = "CON";
                try
                {
            ...
            var name = Parser.Songs != null ? Parser.Songs[0].InternalName + ".mid" : Path.GetFileName(midi);
            ...
            try
            {
                ...
                var fileSize = new FileInfo(midi);
                ...
                for (var i = 0; i < MIDIFile.Events.Tracks; i++)
                {
                    ...
                    if (trackname.Contains("DRUMS") && chkDrums.Checked)
                    {
                    ...
                    else if (trackname.Contains("VOCALS") && chkVocals.Checked)
                    {
        ...
        private void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            {
                if (string.IsNullOrWhiteSpace(ExtractFile(InputFile, Path.GetTempPath() + "temp.mid", FileType.MIDI))) return;
                if (analyzeMoggFileInCONs.Checked)
                {
                    var mogg = ExtractFile(InputFile, Path.GetTempPath() + "temp.mogg", FileType.MOGG);
                    if (string.IsNullOrWhiteSpace(mogg) || !File.Exists(mogg)) return;
                    AnalyzeMoggFiles(new List<string> { mogg }, false);
                    Tools.DeleteFile(mogg);
                }
                if (!analyzePngxboxFile.Checked) return;
                var art = ExtractFile(InputFile, Path.GetTempPath() + "temp.png_xbox", FileType.PNG_XBOX);
            ...
            else switch (Path.GetExtension(InputFile).ToLowerInvariant())
            {
                case ".mid":
                    AnalyzeMIDI(InputFile);
                    break;
                case ".mogg":
                    AnalyzeMoggFiles(new List<string> { InputFile });
                    break;
            }
        }
        ...
        {
            Log("");
            var name = Parser.Songs != null ? Parser.Songs[0].InternalName + "_keep.png_xbox" : Path.GetFileName(art);
        ...
        private void exportToTextFileToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            string name;
            if (!string.IsNullOrWhiteSpace(prodrumfolder))
            {
                name = "_prodrums";
            }
        ...
        private string FormatText(string name, string value)
        {
            ...
            return name.Trim() + (string.IsNullOrWhiteSpace(prodrumfolder) ? ":   "  : "   ") + spacer + "   " + value.Trim();
        }
        ...
        {
            lstStats.Items.Clear();
            Log("Welcome to the Song Analyzer");
            Log("Drag and drop a CON, MIDI or mogg file here");
        ...
        private void MIDIAnalyzer_Shown(object sender, EventArgs e)
        {
            ...
            {
                ResetFormText();
                AnalyzeMoggFiles(new List<string> { argument }, false);
                return;
            }
        ...
        private void openMIDIOrCONFileToolStrip_Click(object sender, EventArgs e)
        {
            ...
            if (VariousFunctions.ReadFileType(ofd.FileName) == XboxFileType.STFS || Path.GetExtension(ofd.FileName).ToLowerInvariant() == ".mid" || Path.GetExtension(ofd.FileName).ToLowerInvariant() == ".mogg")
            {
                ShowWait(true);
        ...
        private void batchAnalyzeMoggFilesToolStrip_Click(object sender, EventArgs e)
        {
            var fbd = new FolderBrowserDialog
                {
                    Description = "Select CON or MOGG Files Folder",
            ...
            MoggFiles = Directory.GetFiles(fbd.SelectedPath, "*.*", SearchOption.TopDirectoryOnly).ToList();
            if (!MoggFiles.Any())
            {
        ...
        private void AnalyzeMoggFiles(IEnumerable<string> files, bool doBatch = true)
        {
            var analyzed = 0;
            var encrypted = 0;
            ...
            foreach (var file in moggs)
            {
                ...
                if (doBatch)
                {
                    Log(counter + ". " + name + ".mogg");
                }
                else
                {
                    Log("Audio File Name", name + ".mogg");
                }
                ...
                Log((doBatch ? "" : "Audio ") + "File Size", fileSize.Length.ToString(CultureInfo.InvariantCulture) + " bytes (" + size + ")");
                using (var fs = File.OpenRead(mogg))
                {
                    using (var br = new BinaryReader(fs))
                    {
                        var version = br.ReadByte();
                        Log("Encryption Type", version == 0x0A ? "None" : "0x" + version.ToString("X2"));
                        if (version != 0x0A)
                        {
                            encrypted++;
                        }
                ...
                if (!Tools.DecM(File.ReadAllBytes(mogg), true, false, DecryptMode.ToMemory))
                {
                ...
                try
                {
                    ...
                    {
                        var stream = Bass.BASS_StreamCreateFile(Tools.GetOggStreamIntPtr(), 0L, Tools.PlayingSongOggData.Length, BASSFlag.BASS_STREAM_DECODE);
                ...
                {
                    Log("Error processing that mogg file:");
                    Log(ex.Message);
                    Bass.BASS_Free();
                    continue;
                }
                ...
                {
                    Log("Drums Channels", Parser.Songs[0].ChannelsDrums == 0 ? "0" : Parser.Songs[0].ChannelsDrums.ToString(CultureInfo.InvariantCulture));
                    Log("Bass Channels", Parser.Songs[0].ChannelsBass == 0 ? "0" : Parser.Songs[0].ChannelsBass.ToString(CultureInfo.InvariantCulture));
                    Log("Guitar Channels", Parser.Songs[0].ChannelsGuitar == 0 ? "0" : Parser.Songs[0].ChannelsGuitar.ToString(CultureInfo.InvariantCulture));
                    Log("Vocals Channels", Parser.Songs[0].ChannelsVocals == 0 ? "0" : Parser.Songs[0].ChannelsVocals.ToString(CultureInfo.InvariantCulture));
                    Log("Keys Channels", Parser.Songs[0].ChannelsKeys == 0 ? "0" : Parser.Songs[0].ChannelsKeys.ToString(CultureInfo.InvariantCulture));
                    Log("Crowd Channels", Parser.Songs[0].ChannelsCrowd == 0 ? "0" : Parser.Songs[0].ChannelsCrowd.ToString(CultureInfo.InvariantCulture));
                    Log("Backing Channels", Parser.Songs[0].ChannelsBacking() == 0 ? "0" : Parser.Songs[0].ChannelsBacking().ToString(CultureInfo.InvariantCulture));
                }
        ...
        private void batchAnalyzeForMissingProDrums_Click(object sender, EventArgs e)
        {
            var fbd = new FolderBrowserDialog
            {
                Description = "Select folder containing CON/MIDI files",
                ShowNewFolderButton = false,
                SelectedPath = Environment.CurrentDirectory,
            };
            ...
            ShowWait(true);
            HaveFile = true;
            prodrumfolder = fbd.SelectedPath;
        ...
        {
            var files = Directory.GetFiles(isSearchingForUnpitchedVocals ? vocalsFolder : prodrumfolder, "*.*").ToList();
            ...
            var folder = prodrumfolder + "\\_NEED_PRO_DRUMS\\";
        ...
        {
            AnalyzeMIDI(midi);
            if (VocalsTalkies > 0 || Harm1Talkies > 0 || Harm2Talkies > 0 || Harm3Talkies > 0)
            {
        ...
        private void searchForOccurrenceOfLyricToolStripMenuItem_Click(object sender, EventArgs e)
        {
            var fbd = new FolderBrowserDialog
            {
                Description = "Select folder of MIDI files",
                ShowNewFolderButton = false,
                SelectedPath = Environment.CurrentDirectory,
            };
            ...
            var midis = Directory.GetFiles(fbd.SelectedPath, "*.mid*");
            if (!midis.Any())
            {
            ...
            LyricSearchFolder = fbd.SelectedPath;
            SongsWithLyric = 0;
            LyricsFound = 0;
            ShowWait(true);
            lstStats.BackgroundImage = null;
